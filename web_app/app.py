...
def login_required(f):
    ...
    def decorated_function(*args, **kwargs):
        ...
        logger.debug(f"Сheck login. User id = {user_id}")
        if user_id is None:
            flash('Пожалуйста, войдите для доступа к этой странице', 'warning')
            return redirect(url_for('login'))
        user = WebUser.get_or_none(id=user_id)
        if not user:
            flash('Сессия невалидна, войдите заново', 'warning')
            session.clear()
            return redirect(url_for('login'))
...
def login():
    if request.method == 'POST':
        ...
        if user and check_password_hash(user.password, password):
            session.permanent = True  # включаем срок жизни сессии
...
@login_required
def objects_list():
    user = WebUser.get_or_none(id=session.get('user_id'))
    if user.id != 0:
...
if __name__ == '__main__':
    logger.remove()  # Удалить все стандартные обработчики
    logger.add(sys.stdout, level="DEBUG")  # Добавить консольный обработчик с уровнем DEBUG
